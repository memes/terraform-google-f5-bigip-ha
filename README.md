...
| <a name="input_instances"></a> [instances](#input\_instances) | An optional map of instances names that will be used to override num\_instances and common paramters. When creating BIG-IP<br>instances the names will correspond to the keys in `instances` variable, and each instance named will receieve the primary<br>and/or Alias IPs associated with the instance. | <pre>map(object({<br>    metadata = map(string)<br>    external = object({<br>      primary_ip    = string<br>      secondary_ips = list(string)<br>    })<br>    mgmt = object({<br>      primary_ip    = string<br>      secondary_ips = list(string)<br>    })<br>    internals = list(object({<br>      primary_ip    = string<br>      secondary_ips = list(string)<br>    }))<br>  }))</pre> | `null` | no |
